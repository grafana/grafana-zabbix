...
func (zabbix *Zabbix) request(ctx context.Context, method string, params ZabbixAPIParams) (*simplejson.Json, error) {
	...
	// (a privilege escalation). The per-user token is regenerated by the auth layer
	// on the next request instead.
	hasPerUserToken := zabbixapi.PerUserTokenFromContext(ctx) != ""
	if hasPerUserToken && notAuthorized {
